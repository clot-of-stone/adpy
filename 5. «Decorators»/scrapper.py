...
def decor(func):
    ...
    def receive_arguments(*args, **kwargs):
        start = time()
        data = func(*args, **kwargs)
        with open(path, 'a', encoding='utf-8') as f:
            try:
                for element in data:
                    f.write(f'Была вызвана функция: {func.__name__}\n'
                            f'Принятые аргументы: {args} и {kwargs}\n'
                            f'Возвращаемое значение: {element}\n'
                            f'***********************\n\n')
            except TypeError:
                pass
        finish = time()
        print(
            ...
            f'составило {round(finish - start, 2)} сек.\n'
            f'Полученные данные сохранены в файл {path}'
        )
        return func(*args, **kwargs)

    return receive_arguments
